...
const ReviewsSection = ({ reviews, onEdit, onDelete }) => {
  const t = useT();
  return (
    <div className="bg-gray-800/30 backdrop-blur-lg rounded-2xl border border-gray-500 border-t-white/30 border-l-white/30 p-6 md:p-8 shadow-2xl">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-white font-special-regular">{t('reviews.myReviewsTitle')}</h2>
      ...
      {reviews.length === 0 ? (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-white mb-2 font-special-regular">{t('reviews.emptyTitle')}</h3>
  ...
  >
    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
      ...
      <span className="text-gray-500 text-sm font-special-regular">
        {formatHumanDateSmart(review.date)}
      </span>
    </div>
    ...
    <div className="flex space-x-3">
      <Link href={`/pages/client/studios/studio-details/${review.studioId}`}>
      ...
      <button
        ...
        className="font-special-regular text-sm bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 rounded-lg"
      >
        {t('reviews.edit')}
      </button>
      <button
        ...
        className="font-special-regular text-sm bg-gray-900 hover:bg-gray-800 text-gray-400 py-2 px-4 rounded-lg"
      >
        {t('reviews.delete')}
      </button>
    </div>
